...
func (t *OpenTelemetryStepFunctionsInterpreter) Interpret(spans []*pb.Span) []*pb.Span {
	...
	for _, span := range spans {
		...
		stateMachineArn, stateMachineArnOk := instrumentation_builders.GetSpanMeta("SFN", span, "aws.request.state.machine.arn")
		...
		if stateMachineArnOk {
			...
			if len(arnParts) >= 7 {
				...
				instrumentation_builders.AwsSpanBuilder(span, stateFunctionName, "State Machine", "step.function", "consumer", urn, arn)
			} else {
