...
func (t *OpenTelemetryS3Interpreter) Interpret(spans []*pb.Span) []*pb.Span {
	...
	for _, span := range spans {
		...
		s3Bucket, s3BucketOk := instrumentations.RetrieveValidSpanMeta(span, "S3", "aws.request.bucket")

		if s3BucketOk {
			var arn = strings.ToLower(fmt.Sprintf("arn:aws:s3:::%s", *s3Bucket))
			var urn = t.CreateServiceURN(arn)

			instrumentations.AwsSpanBuilder(span, *s3Bucket, "S3", "s3", "consumer", urn, arn)
		} else {
