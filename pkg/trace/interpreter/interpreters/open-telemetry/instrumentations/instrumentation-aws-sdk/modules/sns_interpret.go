...
func (t *OpenTelemetrySNSInterpreter) Interpret(spans []*pb.Span) []*pb.Span {
	...
	for _, span := range spans {
		...
		topicArn, topicArnOk := instrumentations.RetrieveValidSpanMeta(span, "SNS", "aws.request.topic.arn")
		...
		if topicArnOk {
			...
			if len(arnParts) >= 6 {
				...
				instrumentations.AwsSpanBuilder(span, topicName, "SNS", "sns", "consumer", urn, arn)
			} else {
