...
func (t *OpenTelemetryStepFunctionsInterpreter) Interpret(spans []*pb.Span) []*pb.Span {
	...
	for _, span := range spans {
		...
		stateMachineArn, stateMachineArnOk := instrumentations.RetrieveValidSpanMeta(span, "SFN", "aws.request.state.machine.arn")
		...
		if stateMachineArnOk {
			...
			if len(arnParts) >= 7 {
				...
				instrumentations.AwsSpanBuilder(span, stateFunctionName, "State Machine", "step.function", "consumer", urn, arn)
			} else {
