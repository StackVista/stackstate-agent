...
`)
	...
	err := chk.Configure(senderManager, handler.NewMockCheckManager(), integration.FakeConfigHash, rawInstanceConfig, []byte(``), "test", "provider")
	assert.Nil(t, err)
...
`)
	...
	assert.Nil(t, err)
	err = check3.Configure(senderManager, handler.NewMockCheckManager(), integration.FakeConfigHash, rawInstanceConfig3, []byte(``), "test", "provider")
	assert.Nil(t, err)
	err = checkSubnet.Configure(senderManager, handler.NewMockCheckManager(), integration.FakeConfigHash, rawInstanceConfigSubnet, []byte(``), "test", "provider")
	assert.Nil(t, err)
