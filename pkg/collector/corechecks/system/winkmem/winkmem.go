...
func (w *KMemCheck) Configure(senderManager sender.SenderManager, checkManager handler.CheckManager, _ uint64, data integration.Data, initConfig integration.Data, source string, provider string) error {
	...
	if err := w.CommonConfigure(senderManager, checkManager, initConfig, data, source, provider); err != nil {
		return err
	}
	cf := Config{
