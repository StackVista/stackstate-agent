...
func TestDefaultConfiguration(t *testing.T) {
	check := NetworkCheck{}
	check.Configure(aggregator.NewNoOpSenderManager(), handler.NewMockCheckManager(), integration.FakeConfigHash, []byte(``), []byte(``), "test", "provider")
...
func TestNetworkCheckWithoutCoreLoader(t *testing.T) {
	...
	err := networkCheck.Configure(mock.GetSenderManager(), handler.NewMockCheckManager(), integration.FakeConfigHash, integration.Data{}, integration.Data{}, "test", "provider")
...
`)
	...
	err := networkCheck.Configure(mockSender.GetSenderManager(), handler.NewMockCheckManager(), integration.FakeConfigHash, rawInstanceConfig, []byte(``), "test", "provider")
	assert.Nil(t, err)
...
`)
	...
	mockSender := mocksender.NewMockSender(networkCheck.ID())
	networkCheck.Configure(mockSender.GetSenderManager(), handler.NewMockCheckManager(), integration.FakeConfigHash, rawInstanceConfig, []byte(``), "test", "provider")
