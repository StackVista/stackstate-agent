...
func (m *mockLongRunningCheck) Configure(senderManger sender.SenderManager, checkManager handler.CheckManager, integrationConfigDigest uint64, config, initConfig integration.Data, source string) error {
	args := m.Called(senderManger, checkManager, integrationConfigDigest, config, initConfig, source)
	return args.Error(0)
}
