...
func (gl *GoCheckLoader) Load(senderManger sender.SenderManager, checkManager handler.CheckManager, config integration.Config, instance integration.Data, instanceIndex int) (check.Check, error) {
	...
	if err := c.Configure(senderManger, checkManager, config.FastDigest(), instance, config.InitConfig, configSource); err != nil {
...
func init() {
	factory := func(sender.SenderManager, handler.CheckManager, option.Option[integrations.Component], tagger.Component) (check.Loader, int, error) {
