...
func TestServiceCollector(t *testing.T) {
	...
	for testCaseNo, tc := range serviceCollectorTestCases(creationTimeFormatted) {
		t.Run(serviceCollectorTestCaseName(tc.testCase), func(t *testing.T) {
			svcCorrelationChannel := make(chan *ServiceSelectorCorrelation)
			componentChannel := make(chan *topology.Component)
			relationChannel := make(chan *topology.Relation)
			collectorChannel := make(chan bool)

			commonCollector := NewTestCommonClusterCollector(MockServiceAPICollectorClient{testCaseNumber: testCaseNo + 1}, componentChannel, relationChannel)
...
func serviceCollectorTestCaseName(baseName string) string {
	return baseName
}

func serviceCollectorTestCases(creationTimeFormatted string) []serviceCollectorTestCase {
