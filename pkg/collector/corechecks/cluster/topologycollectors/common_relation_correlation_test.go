...
func TestRelationCorrelation(t *testing.T) {
	...
	expectedComponents := []*topology.Component{
		expectedPodComponent(pod1),
		expectedPodComponent(pod2),
		expectedConfigMapComponent(configMap1),
		expectedConfigMapComponent(configMap2),
		expectedSecretComponent(secret1),
		expectedSecretComponent(secret2),
		expectedNodeComponent(node1, clusterName),
	}
...
func expectedPodComponent(pod coreV1.Pod) *topology.Component {
	component := &topology.Component{
		ExternalID: fmt.Sprintf("urn:kubernetes:/test-cluster-name:%s:pod/%s", pod.Namespace, pod.Name),
		Type: topology.Type{
			Name: "pod",
		},
		Data: topology.Data{
...
func expectedConfigMapComponent(cm coreV1.ConfigMap) *topology.Component {
	externalID := fmt.Sprintf("urn:kubernetes:/test-cluster-name:%s:configmap/%s", cm.Namespace, cm.Name)
	component := &topology.Component{
	...
	component.SourceProperties = makeSourcePropertiesFullDetails(&cm)
	return component
}
...
func expectedNodeComponent(n coreV1.Node, clusterName string) *topology.Component {
	hostname := fmt.Sprintf("%s-%s", n.Name, clusterName)

	component := &topology.Component{
		ExternalID: fmt.Sprintf("urn:kubernetes:/test-cluster-name:node/%s", n.Name),
		Type: topology.Type{
			Name: "node",
		},
		Data: topology.Data{
	...
	component.SourceProperties = makeSourcePropertiesFullDetails(&n)
	return component
}

func expectedSecretComponent(s coreV1.Secret) *topology.Component {
	externalID := fmt.Sprintf("urn:kubernetes:/test-cluster-name:%s:secret/%s", s.Namespace, s.Name)
	// Mimic the secret collector's data pruning
	prunedSecret := s
	secretDataHash, _ := secure(s.Data)
	...
	component.SourceProperties = makeSourcePropertiesFullDetails(&prunedSecret)
	return component
}
