...
func TestRunClusterCollectors(t *testing.T) {
	...
	opt := KubernetesAPITopologyFactory()
	factory, _ := opt.Get()
	kubernetesTopologyCheck := factory().(*TopologyCheck)
	...
	clusterTopologyCommon := collectors.NewClusterTopologyCommon(instance, clusterType, nil, componentChannel, relationChannel, &version.Info{Major: "1", Minor: "21"})
