...
func (c *TopologyConfig) parse(data []byte) error {
	...
	c.ClusterName = pkgconfigsetup.Datadog().GetString("cluster_name")
	c.CollectTopology = pkgconfigsetup.Datadog().GetBool("collect_kubernetes_topology")
	c.CollectTimeout = pkgconfigsetup.Datadog().GetInt("collect_kubernetes_timeout")
	c.ConfigMapMaxDataSize = pkgconfigsetup.Datadog().GetInt("configmap_max_datasize")
	c.CSIPVMapperEnabled = pkgconfigsetup.Datadog().GetBool("kubernetes_csi_pv_mapper_enabled")
