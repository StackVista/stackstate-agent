...
func TestCommonConfigure(t *testing.T) {
	...
	err := mycheck.CommonConfigure(mockSender.GetSenderManager(), handler.NewMockCheckManager(), nil, []byte(defaultsInstance), "test")
	...
	err = mycheck.CommonConfigure(mockSender.GetSenderManager(), handler.NewMockCheckManager(), nil, []byte(customInstance), "test")
	assert.NoError(t, err)
