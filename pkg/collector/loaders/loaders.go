...
type LoaderFactory func(sender.SenderManager, handler.CheckManager) (check.Loader, error)
...
func LoaderCatalog(senderManager sender.SenderManager, checkManager handler.CheckManager) []check.Loader {
	...
	once.Do(func() {
		...
		for _, k := range keys {
			for _, factory := range factoryCatalog[k] {
				loader, err := factory(senderManager, checkManager)
				if err != nil {
