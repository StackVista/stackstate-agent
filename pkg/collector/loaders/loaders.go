...
type LoaderFactory func(sender.SenderManager, handler.CheckManager, option.Option[integrations.Component], tagger.Component, workloadfilter.Component) (check.Loader, int, error)
...
func LoaderCatalog(senderManager sender.SenderManager, checkManager handler.CheckManager, logReceiver option.Option[integrations.Component], tagger tagger.Component, filter workloadfilter.Component) []check.Loader {
	...
	once.Do(func() {
		...
		for _, factory := range factoryCatalog {
			loader, order, err := factory(senderManager, checkManager, logReceiver, tagger, filter)
			if err != nil {
