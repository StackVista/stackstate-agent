...
func InitCheckScheduler(collector option.Option[collector.Component], senderManager sender.SenderManager, checkManager handler.CheckManager, logReceiver option.Option[integrations.Component], tagger tagger.Component) *CheckScheduler {
	checkScheduler = &CheckScheduler{
		collector:      collector,
		senderManager:  senderManager,
		checkManager:   checkManager,
		configToChecks: make(map[string][]checkid.ID),
		loaders:        make([]check.Loader, 0, len(loaders.LoaderCatalog(senderManager, checkManager, logReceiver, tagger))),
	}
	// add the check loaders
	for _, loader := range loaders.LoaderCatalog(senderManager, checkManager, logReceiver, tagger) {
