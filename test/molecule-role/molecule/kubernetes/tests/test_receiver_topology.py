...
def test_cluster_agent_base_topology(host, ansible_var):
    ...
    def wait_for_cluster_agent_components():
        ...
        assert _component_data(
            ...
            type_name="pod",
            external_id_assert_fn=lambda eid: eid.startswith(
                "urn:kubernetes:/%s:%s:pod/stackstate-agent-" % (cluster_name, namespace)),
            cluster_name=cluster_name,
            identifiers_assert_fn=lambda identifiers: next(
                x for x in identifiers if x.startswith("urn:ip:/%s:" % cluster_name))
        )
...
def test_cluster_agent_pod_mount_volume_relation(host, ansible_var):
    ...
    def wait_for_relation():
        ...
        agent_urn = "urn:kubernetes:/%s:%s:pod/stackstate-agent-.*:container/stackstate-agent->" \
                    % (cluster_name, namespace)
        before_1_21 = _find_mount_relation(
            json_data,
            agent_urn,
        ...
        relation = before_1_21 if before_1_21 else from_1_21
        assert relation.startswith("urn:kubernetes:/%s:%s:pod/stackstate-agent" % (cluster_name, namespace))
