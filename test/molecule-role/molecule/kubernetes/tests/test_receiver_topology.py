...
def test_cluster_agent_base_topology(host, ansible_var):
    ...
    def wait_for_cluster_agent_components():
        ...
        assert _component_data(
            json_data=json_data,
            type_name="pod",
            external_id_assert_fn=lambda eid: eid.startswith("urn:/kubernetes:%s:namespace:%s:pod:stackstate-agent-" % (cluster_name, namespace)),
            cluster_name=cluster_name,
            identifiers_assert_fn=lambda identifiers: next(x for x in identifiers if x.startswith("urn:ip:/%s:" % cluster_name))
        )
        node_agent_container_match = re.compile("urn:/kubernetes:%s:namespace:%s:pod:stackstate-agent-.*:"
        ...
        assert _component_data(
            json_data=json_data,
            type_name="pod",
            external_id_assert_fn=lambda eid: eid.startswith("urn:/kubernetes:%s:namespace:%s:pod:"
                                                             "stackstate-cluster-agent-" % (cluster_name, namespace)),
            cluster_name=cluster_name,
            identifiers_assert_fn=lambda identifiers: next(x for x in identifiers if x.startswith("urn:ip:/%s:" % cluster_name))
        )
        cluster_agent_container_match = re.compile("urn:/kubernetes:%s:namespace:%s:pod:stackstate-cluster-agent-.*"
        ...
        assert _component_data(
            json_data=json_data,
            type_name="service",
            external_id_assert_fn=lambda eid: eid.startswith("urn:/kubernetes:%s:namespace:%s:service:"
        ...
        configmap_match = re.compile("urn:/kubernetes:{}:namespace:{}:configmap:aws-auth"
        ...
        agent_configmap_match = re.compile("urn:/kubernetes:{}:namespace:{}:configmap:sts-agent-config"
        ...
        cluster_agent_configmap_match = re.compile("urn:/kubernetes:{}:namespace:{}:configmap:"
        ...
        # 1 node agent config map sts-agent-config
        agent_configmap_match = re.compile("urn:/kubernetes:{}:namespace:{}:configmap:"
        ...
        replicaset_match = re.compile("urn:/kubernetes:{}:namespace:{}:replicaset:"
        ...
        deployment_match = re.compile("urn:/kubernetes:{}:namespace:{}:deployment:"
                                      "stackstate-cluster-agent".format(cluster_name, namespace))
        assert _find_component(
            json_data=json_data,
        ...
        daemonset_match = re.compile("urn:/kubernetes:{}:namespace:{}:daemonset:"
        ...
        # 1 cronjob hello
        cronjob_match = re.compile("urn:/kubernetes:{}:namespace:{}:cronjob:hello".format(cluster_name, namespace))
        assert _find_component(
            json_data=json_data,
            type_name="cronjob",
        ...
        # 1 job countdown
        job_match = re.compile("urn:/kubernetes:{}:namespace:{}:job:countdown".format(cluster_name, namespace))
        assert _find_component(
            json_data=json_data,
            type_name="job",
        ...
        # volume
        volume_match = re.compile("urn:/kubernetes:{}:namespace:{}:volume:data".format(cluster_name, namespace))
        assert _find_component(
            json_data=json_data,
            type_name="volume",
        ...
        # 1 statefulset mehdb
        statefulset_match = re.compile("urn:/kubernetes:{}:namespace:{}:statefulset:"
        ...
        ingress_match = re.compile("urn:/kubernetes:{}:namespace:{}:ingress:"
        ...
        node_agent_pod_scheduled_match = re.compile("urn:/kubernetes:%s:namespace:%s:pod:stackstate-agent-.*->"
        ...
        ).startswith("urn:/kubernetes:%s:namespace:%s:pod:stackstate-agent-" % (cluster_name, namespace))
        # stackstate-cluster-agent pod is scheduled_on a node (1 time)
        cluster_agent_pod_scheduled_match = re.compile("urn:/kubernetes:%s:namespace:%s:pod:"
        ...
        ).startswith("urn:/kubernetes:%s:namespace:%s:pod:stackstate-cluster-agent" % (cluster_name, namespace))
        # Pod -> Container (encloses)
        # stackstate-agent pod encloses a container (2 times)
        node_agent_container_enclosed_match = re.compile(
            "urn:/kubernetes:%s:namespace:%s:pod:stackstate-agent-.*->"
            "urn:/kubernetes:%s:namespace:%s:pod:stackstate-agent-.*:container:stackstate-agent"
        ...
        cluster_agent_container_enclosed_match = re.compile(
            "urn:/kubernetes:%s:namespace:%s:pod:stackstate-cluster-agent-.*->"
            "urn:/kubernetes:%s:namespace:%s:pod:stackstate-cluster-agent-.*:container:stackstate-cluster-agent"
            % (cluster_name, namespace, cluster_name, namespace))
        pod_encloses_source_id = _relation_data(
            json_data=json_data,
            type_name="encloses",
        ...
        assert re.match("urn:/kubernetes:%s:namespace:%s:pod:stackstate-cluster-agent-.*" % (cluster_name, namespace), pod_encloses_source_id)
        # Pod -> Service (exposes)
        # stackstate-agent exposes stackstate-agent pods (2 times)
        node_agent_service_match = re.compile("urn:/kubernetes:%s:namespace:%s:service:stackstate-cluster-agent->"
                                              "urn:/kubernetes:%s:namespace:%s:pod:stackstate-cluster-agent-.*" %
                                              (cluster_name, namespace, cluster_name, namespace))
        assert _relation_data(
            json_data=json_data,
        ...
        cluster_agent_service_match = re.compile("urn:/kubernetes:%s:namespace:%s:service:stackstate-cluster-agent->"
                                                 "urn:/kubernetes:%s:namespace:%s:pod:stackstate-cluster-agent-.*" %
                                                 (cluster_name, namespace, cluster_name, namespace))
        assert _relation_data(
            json_data=json_data,
        ...
        ).startswith("urn:/kubernetes:%s:namespace:%s:service:pod-service" % (cluster_name, namespace))
        # cluster-agent replicaset controls cluster-agent pod
        replicaset_controls_match = re.compile("urn:/kubernetes:%s:namespace:%s:replicaset:stackstate-cluster-agent-.*"
                                               "->urn:/kubernetes:%s:namespace:%s:pod:stackstate-cluster-agent-.*" %
                                               (cluster_name, namespace, cluster_name, namespace))
        assert _relation_data(
            json_data=json_data,
        ...
        ).startswith("urn:/kubernetes:%s:namespace:%s:replicaset:stackstate-cluster-agent" % (cluster_name, namespace))
        # node-agent daemonset controls node-agent pod
        daemonset_controls_match = re.compile("urn:/kubernetes:%s:namespace:%s:daemonset:stackstate-agent->"
                                              "urn:/kubernetes:%s:namespace:%s:pod:stackstate-agent-.*" %
                                              (cluster_name, namespace, cluster_name, namespace))
        assert _relation_data(
            json_data=json_data,
        ...
        ).startswith("urn:/kubernetes:%s:namespace:%s:daemonset:stackstate-agent" % (cluster_name, namespace))
        # cluster-agent deployment controls replicaset
        deployment_controls_match = re.compile("urn:/kubernetes:%s:namespace:%s:deployment:stackstate-cluster-agent->"
                                               "urn:/kubernetes:%s:namespace:%s:replicaset:stackstate-cluster-agent-.*"
                                               % (cluster_name, namespace, cluster_name, namespace))
        assert _relation_data(
            json_data=json_data,
            type_name="controls",
            external_id_assert_fn=lambda eid:  deployment_controls_match.findall(eid)
        ).startswith("urn:/kubernetes:%s:namespace:%s:deployment:stackstate-cluster-agent" % (cluster_name, namespace))
        #  statefulset controls pod
        statefulset_controls_match = re.compile("urn:/kubernetes:%s:namespace:%s:statefulset:mehdb->"
                                                "urn:/kubernetes:%s:namespace:%s:pod:mehdb-.*" %
                                                (cluster_name, namespace, cluster_name, namespace))
        assert _relation_data(
            json_data=json_data,
            type_name="controls",
            external_id_assert_fn=lambda eid:  statefulset_controls_match.findall(eid)
        ).startswith("urn:/kubernetes:%s:namespace:%s:statefulset:mehdb" % (cluster_name, namespace))
        #  cronjob creates job
        cronjob_creates_match = re.compile("urn:/kubernetes:%s:namespace:%s:cronjob:hello->"
                                           "urn:/kubernetes:%s:namespace:%s:job:hello-.*" %
                                           (cluster_name, namespace, cluster_name, namespace))
        assert _relation_data(
            json_data=json_data,
            type_name="creates",
            external_id_assert_fn=lambda eid:  cronjob_creates_match.findall(eid)
        ).startswith("urn:/kubernetes:%s:namespace:%s:cronjob:hello" % (cluster_name, namespace))
        #  pod claims volume
        pod_claims_volume_match = re.compile("urn:/kubernetes:%s:namespace:%s:pod:mehdb-1->"
                                             "urn:/kubernetes:%s:namespace:%s:volume:data" %
                                             (cluster_name, namespace, cluster_name, namespace))
        assert _relation_data(
            json_data=json_data,
            type_name="claims",
            external_id_assert_fn=lambda eid:  pod_claims_volume_match.findall(eid)
        ).startswith("urn:/kubernetes:%s:namespace:%s:pod:mehdb" % (cluster_name, namespace))
        #  pod claims persistent-volume
        pod_claims_persistent_volume_match = re.compile("urn:/kubernetes:%s:namespace:%s:pod:stackstate-agent-.*->"
        ...
        pod_uses_configmap_match = re.compile("urn:/kubernetes:%s:namespace:%s:pod:stackstate-cluster-agent-.*->"
                                              "urn:/kubernetes:%s:namespace:%s:configmap:sts-clusteragent-config" %
                                              (cluster_name, namespace, cluster_name, namespace))
        assert _relation_data(
            json_data=json_data,
            type_name="uses",
        ...
        pod_uses_configmap_match = re.compile("urn:/kubernetes:%s:namespace:%s:pod:stackstate-agent-.*->"
                                              "urn:/kubernetes:%s:namespace:%s:configmap:sts-agent-config" %
                                              (cluster_name, namespace, cluster_name, namespace))
        assert _relation_data(
            json_data=json_data,
            type_name="uses",
        ...
        ingress_routes_service_match = re.compile("urn:/kubernetes:%s:namespace:%s:ingress:example-ingress->"
                                                  "urn:/kubernetes:%s:namespace:%s:service:banana-service" %
                                                  (cluster_name, namespace, cluster_name, namespace))
        assert _relation_data(
            json_data=json_data,
            type_name="routes",
            external_id_assert_fn=lambda eid:  ingress_routes_service_match.findall(eid)
        ).startswith("urn:/kubernetes:%s:namespace:%s:ingress:example-ingress" % (cluster_name, namespace))
        # stackstate-cluster-agent Container mounts Volume  stackstate-cluster-agent-token
        container_mounts_volume_match = re.compile(
            "urn:/kubernetes:%s:namespace:%s:pod:stackstate-cluster-agent-.*:container:stackstate-cluster-agent->"
            "urn:/kubernetes:%s:namespace:%s:volume:stackstate-cluster-agent-token-.*"
            % (cluster_name, namespace, cluster_name, namespace)
        )
        assert _relation_data(
            json_data=json_data,
        ...
        agent_container_mounts_volume_match = \
            re.compile("urn:/kubernetes:%s:namespace:%s:pod:stackstate-agent-.*:container:stackstate-agent->"
                       "urn:/kubernetes:%s:namespace:%s:volume:stackstate-agent-token-.*" %
                       (cluster_name, namespace, cluster_name, namespace))
        assert _relation_data(
            json_data=json_data,
            type_name="mounts",
        ...
        job_controls_match = re.compile("urn:/kubernetes:%s:namespace:%s:job:hello-.*->"
                                        "urn:/kubernetes:%s:namespace:%s:pod:hello-.*" %
                                        (cluster_name, namespace, cluster_name, namespace))
        assert _relation_data(
            json_data=json_data,
            type_name="controls",
            external_id_assert_fn=lambda eid:  job_controls_match.findall(eid)
        ).startswith("urn:/kubernetes:%s:namespace:%s:job:hello" % (cluster_name, namespace))
