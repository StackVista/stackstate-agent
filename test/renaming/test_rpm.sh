...
if [ "$#" -ne 1 ];  then
  ...
  echo "Example: ./test.sh pkg/stackstate-agent_1.0.0.git.3.d95a036-1_amd64.rpm"
  exit 1
fi
...
cp ${ARTIFACT} agent.rpm

rm -rf unpacked
mkdir unpacked
cd unpacked

rpm2cpio ../agent.rpm | cpio -idmv
...
rm ./opt/stackstate-agent/embedded/.installed_by_pkg.txt
rm ./opt/stackstate-agent/embedded/.py_compiled_files.txt
