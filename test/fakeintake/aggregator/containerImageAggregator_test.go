...
func TestNewContainerImagePayloads(t *testing.T) {
	...
	t.Run("parseContainerImagePayload valid body should parse payloads", func(t *testing.T) {
		...
		expectedTags := []string{"git.repository_url:https://github.com/StackVista/stackstate-agent", "os_name:linux", "architecture:amd64", "image_id:gcr.io/datadoghq/agent@sha256:c08324052945874a0a5fb1ba5d4d5d1d3b8ff1a87b7b3e46810c8cf476ea4c3d", "image_name:gcr.io/datadoghq/agent", "short_image:agent", "image_tag:7.50.1"}
		assert.Equal(t, expectedTags, payloads[0].GetTags())

		assert.Equal(t, "gcr.io/datadoghq/dogstatsd", payloads[2].name())
		expectedTags = []string{"os_name:linux", "architecture:amd64", "git.repository_url:https://github.com/StackVista/stackstate-agent", "image_id:gcr.io/datadoghq/dogstatsd@sha256:1d85318569be685c6aeb98df85da995050feda8e55e8f5e122ad96689e6205cd", "image_name:gcr.io/datadoghq/dogstatsd", "short_image:dogstatsd", "image_tag:7.50.1"}
