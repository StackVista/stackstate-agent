...
build do
  if windows?
    ...
    command curl_cmd
    command "mv #{binary} #{Omnibus::Config.source_dir()}/datadog-agent/src/github.com/StackVista/stackstate-agent/bin/agent/#{target_binary}"
  else
    process_agent_arch = ENV['ARCH']
    binary = "process-agent-#{process_agent_arch}-#{process_agent_version}"
