...
build do
  ...
  block do
    ...
    # --no-emit-{index-url,find-links}: defence in depth — see STAC-24642.
    if windows?
...
#       command "#{pip} install --no-deps .", :env => nix_build_env, :cwd => "#{project_dir}/stackstate_checks_downloader"
      command "#{python} -m piptools compile --generate-hashes --no-emit-index-url --no-emit-find-links --output-file #{install_dir}/#{agent_requirements_file} #{static_reqs_out_file}", :env => nix_build_env
    end
