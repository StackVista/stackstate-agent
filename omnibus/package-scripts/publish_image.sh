...
if [ -n "$EXTRA_TAG" ]; then
    ...
    if [ "${SLIM_INSTALLED}" = "true" ]; then
        ...
        # regctl
        apk add curl
        ~/.slim/bin/slim inst --include-last-image-layers 4 --instrumented-image-connector "${SLIM_CONNECTOR_ID}" --hardened-image-connector "${SLIM_CONNECTOR_ID}" "${DOCKER_EXTRA_TAG}"
    fi
