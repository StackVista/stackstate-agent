...
func PyTestRun(ctx PytestContext, watch bool) error {
	cmd := buildPyTestCmd(ctx, watch)
	log.Printf("running Pytest cmd: %s", cmd)
	return runPyTestCmd(context.Background(), cmd)
}

func buildPyTestCmd(ctx PytestContext, watch bool) *exec.Cmd {
	var args []string
	...
	//TODO select tests by expression
	//args = append(args, fmt.Sprintf("%s %s", ExpressionFlag, "test_agent_integration_sample_metrics"))
