...
/*! \file rawmetrics.h
    \brief RtLoader rawmetrics builtin header file.
...
/*! \fn PyMODINIT_FUNC PyInit_raw_metrics(void)
    ...
    PyMethodDef definition in `rawmetrics.c` with the corresponding C-implemented python
    methods. A fresh reference to the module is created here. This function is python3
    only.
*/
/*! \fn void Py2_init_raw_metrics()
    \brief Initializes the raw metrics builtin python module.
    The rawmetrics python builtin is created and registered here as per the module_def
    PyMethodDef definition in `rawmetrics.c` with the corresponding C-implemented python
    methods. A fresh reference to the module is created here. This function is python2
    only.
*/
/*! \fn void _set_submit_metrics_raw_data(cb_submit_metrics_raw_data_t)
    \brief Sets the raw metrics data callback to be used by rtloader for submitting raw metric data.
    \param object A function pointer with cb_submit_metrics_raw_data_t function prototype to the
    callback function.
...
#define RAW_METRICS_MODULE_NAME "rawmetrics"
