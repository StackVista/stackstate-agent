...
def generate_install(ctx, test_repo=False):
    ...
    ctx.run("sed -e 's/$DEBIAN_REPO/https:\/\/{0}.s3.amazonaws.com/' ./cmd/agent/install_script.sh > ./cmd/agent/install.sh".format(deb_repo))
    ctx.run("sed -e 's/$YUM_REPO/https:\/\/{0}.s3.amazonaws.com/' ./cmd/agent/install.sh > ./cmd/agent/install.sh".format(yum_repo))
