...
class AgentTestingBase:
    ...
    def ubuntu_get_agent_version(self) -> Optional[str]:
        try:
            _, stdout, stderr = self.client.exec_command(f'apt-cache madison stackstate-agent', timeout=15)
            read_stdout: str = stdout.read().decode()
            agent_full_madison = read_stdout.split(" | ")[1]
            agent_version = agent_full_madison.split(r'.git.')[0]
    ...
    def ubuntu_reinstall_agent(self,
                               ansible_var: Callable[[str], str]) -> None:
        command = f"export LOG_PAYLOADS=true; \
        export STS_LOG_LEVEL=debug; \
        export STS_API_KEY='{ansible_var('sts_api_key')}'; \
        export STS_URL='{ansible_var('sts_url')}'; \
        export STS_HOSTNAME='{ansible_var('inventory_hostname')}'; \
        export CODE_NAME='{ansible_var('agent_current_branch')}'; \
        export SKIP_SSL_VALIDATION=false; \
        curl -o- {ansible_var('agent_repo_url')}/install.sh | bash"

        print(command)

        # # export LOG_PAYLOADS=true; export STS_LOG_LEVEL=debug; export STS_API_KEY='beest-manual-qa-process--agent-upgrade'; export STS_URL='https://beest-manual-qa-process--agent-upgrade.sandbox-main.sandbox.stackstate.io'; export STS_HOSTNAME='local'; export SKIP_SSL_VALIDATION=false; curl -o- https://stackstate-agent-2.s3.amazonaws.com/install.sh | bash
        # _, stdout, stderr = self.client.exec_command(f"export LOG_PAYLOADS=true;"
        #                                              f"export STS_LOG_LEVEL=debug;"
        #                                              f"export AGENT_REPO_URL='{ansible_var('agent_repo_url')}';"
        #                                              f"export STS_API_KEY='{ansible_var('sts_api_key')}';"
        #                                              f"export STS_URL='{ansible_var('sts_url')}';"
        #                                              f"export STS_HOSTNAME='{ansible_var('inventory_hostname')}';"
        #                                              f"export SKIP_SSL_VALIDATION=false;"
        #                                              f"curl -o- $AGENT_REPO_URL/install.sh | bash", timeout=360)
        # print("----------------")
        # print("----------------")
    ...
    def yum_get_stackstate_agent_version(self) -> Optional[str]:
        try:
            _, stdout, stderr = self.client.exec_command(f'rpm -q stackstate-agent', timeout=15)
            read_stdout: str = stdout.read().decode()
            agent_rmp_q_full = read_stdout\
                .replace("stackstate-agent-", "")\
                .replace("stackstate-agent", "")
            agent_version = agent_rmp_q_full.split(r'.git.')[0]
