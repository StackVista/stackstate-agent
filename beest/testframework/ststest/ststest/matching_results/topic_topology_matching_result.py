...
from ..primitive_matchers import ComponentMatcher, RelationMatcher, DeleteMatcher
from ..matches import TopicTopologyMatch


class TopicTopologyMatchingResult(MatchingResult):
    def __init__(self,
        ...
        super(TopicTopologyMatchingResult, self).__init__()
    ...
    def assert_exact_match(self, matching_graph_name=None, allow_duplicate_matches: bool = True) -> TopicTopologyMatch:
        if len(self._topology_matches) == 1:
            return self._topology_matches[0]
        elif len(self._topology_matches) > 1 and allow_duplicate_matches:
            return self._topology_matches[0]
