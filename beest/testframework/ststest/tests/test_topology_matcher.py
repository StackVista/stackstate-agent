...
from ststest.ststest import TopologyMatcher, TopologyMatch, TopicTopologyMatcher, TopicTopologyMatch
...
class TestTopicTopologyMatcher(TestCase):
    def test_simple_positive(self):
        ...
        result = matcher.find(input_topology.topology())
        match = result.assert_exact_match(matching_graph_name=self._testMethodName, matching_graph_upload=False)
        self.assertEqual(TopicTopologyMatch(
            components={'A': input_topology.get('a'),
                        'B': input_topology.get('b')},
            relations={'A_TO_B': input_topology.get('a>before>b')},
    ...
    def test_complex_positive(self):
        input_topology = TopicTopologyFixture("a1,a2,b1,del a10,b2,c1,c2,a1>to>b1,a1>to>b2,a2>to>b1,b1>to>a2,b2>to>c1,"
                                              "del c5")
        matcher = TopicTopologyMatcher() \
            .component("A", name="a.") \
            .component("B", name="b.") \
...
desired topology was not matched:
 ...
 """.strip())
    ...
    def test_simple_wrong_delete(self):
        ...
        result = matcher.find(TopicTopologyFixture("a,b").topology())
    ...
    def test_topology_fixture(self):
        ...
        self.assertEqual(TopicTopologyResult(
            ...
            relations=[relation_fixture(3, 1, 2, 'to'),
                       ...
                       relation_fixture(7, 4, 2, 'alsoto')],
            deletes=[]
        ), TopicTopologyFixture("a,b,a>to>b,c,b>goes>c,c>backto>a,c>alsoto>b").topology())
        ...
        ), TopicTopologyFixture("del X,del Y,del Z").topology())
        ...
        self.assertEqual(TopicTopologyResult(
            ...
            relations=[relation_fixture(3, 1, 2, 'to'),
                       ...
                       relation_fixture(7, 4, 2, 'alsoto')],
            deletes=[delete_fixture(1, 'X'), delete_fixture(2, 'Y'), delete_fixture(3, 'Z')]
        ), TopicTopologyFixture("a,b,a>to>b,c,b>goes>c,c>backto>a,c>alsoto>b,del X,del Y,del Z").topology())
