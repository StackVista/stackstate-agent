...
from ststest import TopologyMatcher, TopicTopologyMatcher, TopicTopologyMatch
...
class TestTopicTopologyMatcher(TestCase):
    def test_simple_positive(self):
        input_topology = TopicTopologyFixture("a,b,c,a>before>b,c>before>b,del y")
        matcher = TopicTopologyMatcher() \
            .component("A", name="a") \
            .component("B", name="b") \
            .one_way_direction("A", "B", type="before") \
            .delete(id="y")
    ...
    def test_repeated(self):
        node1 = f"node-{random.randint(100, 200)}"
        node2 = f"node-{random.randint(100, 200)}"
        input_topology = TopicTopologyFixture(f"cluster,{node1},{node2},{node1}>belongs>cluster,"
                                              f"{node2}>belongs>cluster")
        matcher = TopicTopologyMatcher() \
            .component("C", name=r"cluster") \
            .repeated(2,
                      lambda m: m
    ...
    def test_repeated_complex(self):
        ...
        input_topology = TopicTopologyFixture(','.join([
            node1, node2, pod1, pod2,
            daemonset, configmap, cluster,
            f"{pod1}>scheduled_on>{node1}",
            f"{pod2}>scheduled_on>{node2}",
    ...
    def test_complex_positive(self):
        input_topology = TopicTopologyFixture("a1,a2,b1,b2,c1,c2,a1>to>b1,a1>to>b2,a2>to>b1,b1>to>a2,b2>to>c1,del y")
        matcher = TopicTopologyMatcher() \
            .component("A", name="a.") \
            .component("B", name="b.") \
            .component("C", name="c.") \
    ...
    def test_render_dot(self):
        input_topology = TopicTopologyFixture(
            'a,b,c1,c2,d1,d2,e,a>to>b,b>to>c1,b>to>c2,c1>to>d1,c2>to>d2,d1>to>e,d2>to>e,f1,f2,e>to>f1,e>to>f2'
        )
        matcher = TopicTopologyMatcher() \
            .component('A', name='a') \
...
'''
    ...
    def test_ambiguous_match_failure(self):
        input_topology = TopicTopologyFixture("a1,a2,b1,b2,c1,c2,a1>to>b2,b2>to>c1,a2>to>b2,b1>to>c1,b1>to>c2")
        matcher = TopicTopologyMatcher() \
            .component("A", name="a.") \
            .component("B", name="b.") \
            .component("C", name="c.") \
...
""".strip())
    ...
    def test_simple_wrong_relation(self):
        ...
        result = matcher.find(TopicTopologyFixture("a,b,c,b>before>c,a>after>b").topology())
...
desired topology was not matched:
 ...
 """.strip())
    ...
    def test_simple_wrong_delete(self):
        ...
        result = matcher.find(TopicTopologyFixture("a,b,c,a>before>b").topology())
        ...
        self.assertEqual(exception_message,
                         """
desired topology was not matched:
	delete y[id~=y] was not found
 """.strip())

    def test_topology_fixture(self):
        self.assertEqual(TopicTopologyResult(
            components=[component_fixture(1, 'a')],
            relations=[]
        ), TopicTopologyFixture("a").topology())

        self.assertEqual(TopicTopologyResult(
            components=[],
            relations=[],
            deletes=[delete_fixture(1, 'y')]
        ), TopicTopologyFixture("del y").topology())

        self.assertEqual(TopicTopologyResult(
            components=[component_fixture(1, 'a', outgoing=[3]),
                        component_fixture(2, 'b', incoming=[3])],
            relations=[relation_fixture(3, 1, 2, 'to')]
        ), TopicTopologyFixture("a,b,a>to>b").topology())
        ...
        self.assertEqual(TopicTopologyResult(
            ...
            relations=[relation_fixture(3, 1, 2, 'to'),
                       ...
                       relation_fixture(7, 4, 2, 'alsoto')],
            deletes=[delete_fixture(1, 'y')]
        ), TopicTopologyFixture("a,b,a>to>b,c,b>goes>c,c>backto>a,c>alsoto>b,del y").topology())

        with self.assertRaises(KeyError, msg="forward reference should be rejected in constructor"):
            TopicTopologyFixture("a,a>to>b,b")
