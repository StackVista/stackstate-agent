...
from stackstate_checks.base.checks.v2.transactional_agent_check import AgentCheckV2, TransactionalAgentCheck
from stackstate_checks.base import StackPackInstance, MetricStream, MetricHealthChecks, \
...
class AgentV2IntegrationTransactionalSampleCheck(TransactionalAgentCheck):
    ...
    def get_instance_key(self, instance):
        return StackPackInstance("agent-v2-integration-transactional", "sample")
    ...
    def transactional_check(self, instance, transactional_state, persistent_state):
        ...
        self.count("{}_transactional".format(check_identifier), int(transactional_state["transaction_counter"]),
                   tags=[check_identifier])
...
def agent_v2_integration_stateful_base(self, instance, persistent_state):
    ...
    self.count("{}_stateful".format(check_identifier), int(persistent_state["persistent_counter"]),
               tags=[check_identifier])
