resource "local_file" "ansible_inventory" {
  filename        = "${path.module}/ansible_inventory"
  content         = <<INVENTORY
[local]
localhost ansible_connection=local

[splunk]
${module.ec2_splunk.splunk_ip} ansible_connection=ssh ansible_ssh_private_key_file=${local_file.splunk_id_rsa.filename} ansible_user=ubuntu ansible_password=

[all:vars]
yard_id="${var.yard_id}"
aws_region="${var.aws_default_region}"
INVENTORY
  file_permission = "0777"
}

resource "local_file" "splunk_id_rsa" {
  // path.cwd return the full path which is needed in the ansible_inventory
  filename        = "${path.cwd}/agent_id_rsa"
  content         = module.ec2_splunk.ssh_key
  file_permission = "0600"
}
