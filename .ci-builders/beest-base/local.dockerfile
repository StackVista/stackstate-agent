...
ARG GITLAB_PACKAGE_REGISTRY_PYPI_URL
ARG GITLAB_PACKAGE_REGISTRY_USER
ARG GITLAB_PACKAGE_REGISTRY_TOKEN
RUN mkdir ~/.pip/ && touch ~/.pip/pip.conf && \
    echo "[global]" > ~/.pip/pip.conf && \
    echo "extra-index-url = https://${GITLAB_PACKAGE_REGISTRY_USER}:${GITLAB_PACKAGE_REGISTRY_TOKEN}@${GITLAB_PACKAGE_REGISTRY_PYPI_URL}" >> ~/.pip/pip.conf
