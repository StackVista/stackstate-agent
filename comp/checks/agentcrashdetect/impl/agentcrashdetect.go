...
func (wcd *AgentCrashDetect) Configure(senderManager sender.SenderManager, checkManager handler.CheckManager, _ uint64, data integration.Data, initConfig integration.Data, source string, provider string) error {
	err := wcd.CommonConfigure(senderManager, checkManager, initConfig, data, source, provider)
	if err != nil {
		return err
	}
