...
func (c *CheckWrapper) Configure(senderManager sender.SenderManager, checkManager handler.CheckManager, integrationConfigDigest uint64, config, initConfig integration.Data, source string, provider string) error {
	if c.senderManager == nil {
		c.senderManager = senderManager
	}
	return c.inner.Configure(c.senderManager, checkManager, integrationConfigDigest, config, initConfig, source, provider)
}
