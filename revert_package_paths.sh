...
find $CI_PROJECT_DIR -type d -name .git -prune -o -type f -name "*.go" -exec sed -i 's/StackVista\/stackstate-agent/DataDog\/datadog-agent/g' {} +

# This line is used to fix the package import paths in go.mod files in the agent codebase.
find $CI_PROJECT_DIR -type d -name .git -prune -o -type f -name "*.mod" -exec sed -i 's/StackVista\/stackstate-agent/DataDog\/datadog-agent/g' {} +
