...
| secret-generic-connector | Not packaged by STS; use a custom `secret_backend_command` such as `/readsecret.py` instead |
...
| C5 | freetds, msodbcsql18, unixodbc, secret-generic-connector.rb; STAC-25211 later removed the connector from STS packaging because `secret_backend_type` is not a supported STS chart surface |
